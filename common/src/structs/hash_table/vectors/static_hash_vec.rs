    ...
    for StaticHashVec<K, V>
{
    ...
    fn get(&mut self, index: usize, key: &K) -> Option<KeyValue<K, V>> {
        ...
        while i < self.data[index].len() {
            if self.data[index][i].key == *key {
                return Some(self.data[index][i].clone());
            }
            i += 1;
        }
        None
    ...
    fn update(
        ...
        match item_index {
            Some(i) => {
                let old_value = self.data[index][i].value.clone();
                self.data[index][i].value = value;
                Some(KeyValue::new(key.clone(), old_value))
            }
            None => None,
        }
    }
    ...
    for StaticHashVec<K, V>
{
    ...
    fn get_by_index(
        ...
        if value_index >= self.data[index].len() {
            ...
            Some(self.data[index][value_index].clone())
        }
    }
