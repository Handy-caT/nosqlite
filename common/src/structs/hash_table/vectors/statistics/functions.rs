use crate::structs::hash_table::vectors::hash_vec::{
...
pub fn statistics_add_actions<K, V, M>(hash_vec: &mut M, index: usize)
where
...
/// * `hash_vec` - [`HashVec`] to update
/// * `index` - index of bucket
pub fn statistics_remove_actions<K, V, M>(hash_vec: &mut M, index: usize)
where
    M: HashVec<K, V> + InternalStatistics<K, V>,
{
    hash_vec.get_statistics_mut().size -= 1;
